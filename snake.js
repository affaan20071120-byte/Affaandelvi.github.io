...
if (ctx) {
  ...
  function placeFood(){ food = { x: rand(canvas.width/size)|0, y: rand(canvas.height/size)|0 }; }
  ...
  function step(){
    ...
    if(snake.some((s,i)=> i && s.x===head.x && s.y===head.y)){ end(); return; }

    if(head.x===food.x && head.y===food.y){ score++; document.getElementById('score').textContent = score; snake.unshift(head); placeFood(); }
  ...
  function end(){ clearInterval(loop); alert('Game over! Score: ' + score); }
  function start(){ clearInterval(loop); score=0; snake=[{x:5,y:5}]; dir='RIGHT'; placeFood(); document.getElementById('score').textContent = score; loop=setInterval(step, 110); }
